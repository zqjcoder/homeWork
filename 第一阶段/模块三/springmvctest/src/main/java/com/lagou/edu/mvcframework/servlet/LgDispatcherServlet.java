    ...
    private void initHandlerMapping() {
        ...
        for(Map.Entry<String,Object> entry: ioc.entrySet()) {
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                Set<String> methodUsers = new HashSet<>();
                methodUsers.addAll(Arrays.asList(annotation1.value()));

                // 把method所有信息及url封装为一个Handler
                Handler handler = new Handler(entry.getValue(),method, Pattern.compile(url));
                handler.setClassUsers(classUsers);
                handler.setMethodUsers(methodUsers);
...
//        method.invoke() //
        ...
        for(Map.Entry<String,String[]> param: parameterMap.entrySet()) {
            ...
            if (param.getKey().equalsIgnoreCase("username")) {
                userName = param.getValue()[0];
            }
        ...
        try {
            Set<String> classUsersUsers = handler.getClassUsers();
            Set<String> methodUsers = handler.getMethodUsers();
            if (null == userName || !(classUsersUsers.contains(userName) || methodUsers.contains(userName))) {
                resp.getOutputStream().write("NO PERMISSION!".getBytes());
                return;
            }
