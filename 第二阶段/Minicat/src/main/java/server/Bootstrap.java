        ...
        try {
            ...
            for (int i = 0; i < selectNodes.size(); i++) {
                ...
                for (Element element : serviceList) {
                    ...
                    String port1 = connectorElement.attributeValue("port");
                    port = Integer.valueOf(port1);
                    List<Element> engineList = connectorElement.selectNodes("//Engine");
                    for (Element element1 : engineList) {
                        List<Element> hostList = element1.selectNodes("//Host");
                        for (Element element2 : hostList) {
                            //读取到appBase路径
                            String appBase = element2.attributeValue("appBase");
            ...
            //根据appBase路劲去扫描该路径下的包及servlet,封装相关对象
            BuildMapper.buildMapper(mapper,servletMap);
